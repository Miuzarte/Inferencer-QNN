...
import (
	...
	"Inferencer/ortpurego/internal/sys"
)
...
func (e *Engine) initApi() error {
	...
	// EP registration
	purego.RegisterFunc(&e.funcs.registerExecutionProviderLibrary, e.api.RegisterExecutionProviderLibrary)

	// EP devices
	purego.RegisterFunc(&e.funcs.getEpDevices, e.api.GetEpDevices)
	purego.RegisterFunc(&e.funcs.epDeviceEpName, e.api.EpDevice_EpName)
	purego.RegisterFunc(&e.funcs.sessionOptionsAppendExecutionProviderV2, e.api.SessionOptionsAppendExecutionProvider_V2)
	...
	purego.RegisterFunc(&e.funcs.addSessionConfigEntry, e.api.AddSessionConfigEntry)
	...
	// io binding
	purego.RegisterFunc(&e.funcs.createIoBinding, e.api.CreateIoBinding)
	purego.RegisterFunc(&e.funcs.releaseIoBinding, e.api.ReleaseIoBinding)
	purego.RegisterFunc(&e.funcs.bindInput, e.api.BindInput)
	purego.RegisterFunc(&e.funcs.bindOutput, e.api.BindOutput)
	purego.RegisterFunc(&e.funcs.bindOutputToDevice, e.api.BindOutputToDevice)
	purego.RegisterFunc(&e.funcs.getBoundOutputNames, e.api.GetBoundOutputNames)
	purego.RegisterFunc(&e.funcs.getBoundOutputValues, e.api.GetBoundOutputValues)
	purego.RegisterFunc(&e.funcs.clearBoundInputs, e.api.ClearBoundInputs)
	purego.RegisterFunc(&e.funcs.clearBoundOutputs, e.api.ClearBoundOutputs)
	purego.RegisterFunc(&e.funcs.runWithBinding, e.api.RunWithBinding)
	purego.RegisterFunc(&e.funcs.synchronizeBoundInputs, e.api.SynchronizeBoundInputs)
	purego.RegisterFunc(&e.funcs.synchronizeBoundOutputs, e.api.SynchronizeBoundOutputs)
	purego.RegisterFunc(&e.funcs.createMemoryInfo, e.api.CreateMemoryInfo)
	purego.RegisterFunc(&e.funcs.createMemoryInfo_V2, e.api.CreateMemoryInfo_V2)
